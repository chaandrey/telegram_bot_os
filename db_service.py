...
def update_record(chat_id, collection, updated_price, timestamp):
    """Updating record with new price."""
    connec = connection_to_db()   # need to update timestamp as well.
    connec.execute(f"UPDATE users3 SET current_price=?, timestamp =? WHERE chat_id=? AND collection =?", (updated_price,timestamp,chat_id,collection))
    connec.commit()
